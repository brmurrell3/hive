...
func (m *mockStore) ModifyAgent(id string, fn func(*state.AgentState) error) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func TestCrashRecovery_DetectsDeadProcess(t *testing.T) {
	...
	cr, err := NewCrashRecovery(RecoveryConfig{
		Store:  store,
		Logger: logger,
	})
	if err != nil {
...
func TestGracefulShutdown_StopsAllAgents(t *testing.T) {
	...
	gs, err := NewGracefulShutdown(ShutdownConfig{
		Store:     store,
		VMManager: vmMgr,
		Logger:    logger,
...
func TestResourceMonitor_StartsAndStops(t *testing.T) {
	...
	rm, err := NewResourceMonitor(MonitorConfig{
		Store:         store,
		Metrics:       metrics,
		Logger:        logger,
...
func TestComputeUsage_LocalNodeUsesRealData(t *testing.T) {
	...
	// Create a node whose ID matches the configured LocalNodeID so it is
...
func TestComputeUsage_RemoteNodeUsesEstimate(t *testing.T) {
	...
	// Create a node whose ID does not match the configured LocalNodeID,
	// so it is treated as a remote node and uses estimates.
