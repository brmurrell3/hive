...
FIRECRACKER_KERNEL_URL_AMD64="https://s3.amazonaws.com/spec.ccfc.min/img/quickstart_guide/x86_64/kernels/vmlinux.bin"
FIRECRACKER_KERNEL_URL_ARM64="https://s3.amazonaws.com/spec.ccfc.min/img/quickstart_guide/aarch64/kernels/vmlinux.bin"
...
download_binaries() {
    ...
    # GoReleaser strips the 'v' prefix: tag v1.0.0 -> version 1.0.0
    local version_no_v="${VERSION#v}"
    local tarball_name="hive-${version_no_v}-linux-${DETECTED_ARCH}.tar.gz"
    local tarball_url="${DOWNLOAD_URL_BASE}/${tarball_name}"
    local tarball_dest="${TMPDIR_INSTALL}/${tarball_name}"

    download_file "${tarball_url}" "${tarball_dest}" "hive tarball (${tarball_name})"
    ...
    if curl -fsSL --retry 1 -o "${checksum_file}" "${checksum_url}" 2>/dev/null; then
        ...
        if [ "${DRY_RUN}" != true ]; then
            local expected
            expected="$(grep "${tarball_name}" "${checksum_file}" 2>/dev/null | awk '{print $1}' || true)"
            if [ -n "${expected}" ]; then
                local actual
                actual="$(sha256sum "${tarball_dest}" | awk '{print $1}')"
                if [ "${expected}" = "${actual}" ]; then
                    log_ok "Checksum verified: ${tarball_name}"
                else
                    die "Checksum mismatch for ${tarball_name}: expected ${expected}, got ${actual}"
                fi
            else
                log_warn "Tarball not found in checksums.txt. Skipping checksum verification."
            fi
        fi
    else
    ...
    # Optional cosign verification (graceful fallback if cosign not installed).
    if command -v cosign >/dev/null 2>&1; then
        log_info "Verifying cosign signature..."
        local sig_url="${DOWNLOAD_URL_BASE}/checksums.txt.sig"
        local pem_url="${DOWNLOAD_URL_BASE}/checksums.txt.pem"
        local sig_file="${TMPDIR_INSTALL}/checksums.txt.sig"
        local pem_file="${TMPDIR_INSTALL}/checksums.txt.pem"
        if curl -fsSL --retry 1 -o "${sig_file}" "${sig_url}" 2>/dev/null && \
           curl -fsSL --retry 1 -o "${pem_file}" "${pem_url}" 2>/dev/null; then
            if cosign verify-blob "${checksum_file}" \
                --signature "${sig_file}" \
                --certificate "${pem_file}" \
                --certificate-oidc-issuer https://token.actions.githubusercontent.com \
                --certificate-identity-regexp "github\\.com/${GITHUB_REPO}" 2>/dev/null; then
                log_ok "Cosign signature verified."
            else
                log_warn "Cosign signature verification failed. Continuing anyway."
            fi
        else
            log_warn "Cosign signature/certificate not available for this release."
        fi
    else
        log_info "cosign not installed. Skipping signature verification."
        log_info "  Install cosign for supply-chain verification: https://docs.sigstore.dev/cosign/system_config/installation/"
    fi

    # Extract binaries from tarball.
    if [ "${DRY_RUN}" != true ]; then
        log_info "Extracting binaries from tarball..."
        tar -xzf "${tarball_dest}" -C "${TMPDIR_INSTALL}" --strip-components=1
        log_ok "Extracted binaries."
    fi
}
...
download_images() {
    ...
    # Download rootfs image from the Hive release (gzipped).
    local rootfs_gz_name="hive-rootfs-base-${DETECTED_ARCH}.ext4.gz"
    local rootfs_url="${DOWNLOAD_URL_BASE}/${rootfs_gz_name}"
    ...
    if [ -f "${rootfs_dest}" ]; then
        ...
        local rootfs_gz_dest="${TMPDIR_INSTALL}/${rootfs_gz_name}"
        log_info "Downloading rootfs image..."
        log_info "  URL:  ${rootfs_url}"
        log_info "  Dest: ${rootfs_dest}"
        if [ "${DRY_RUN}" = true ]; then
            log_info "[DRY-RUN] Would download ${rootfs_url} and decompress to ${rootfs_dest}"
        elif curl -fSL --retry 3 --retry-delay 2 --progress-bar -o "${rootfs_gz_dest}" "${rootfs_url}" 2>/dev/null; then
            gzip -d "${rootfs_gz_dest}"
            local rootfs_raw="${rootfs_gz_dest%.gz}"
            mv "${rootfs_raw}" "${rootfs_dest}"
            chown "${HIVE_USER}:${HIVE_GROUP}" "${rootfs_dest}"
            chmod 644 "${rootfs_dest}"
            log_ok "Downloaded and decompressed rootfs image."
        else
            rm -f "${rootfs_gz_dest}"
