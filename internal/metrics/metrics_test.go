...
func TestCollector_InvocationLatency(t *testing.T) {
	...
	// Register the capability so it is not bucketed under "other".
	c.RegisterCapability("answer-questions")
...
func TestCollector_PrometheusFormat(t *testing.T) {
	...
	c.IncErrorCount("timeout")
	c.SetNATSConnected(true)

	// Re-render after adding new metrics.
	req = httptest.NewRequest(http.MethodGet, "/metrics", nil)
	rec = httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	resp = rec.Result()
