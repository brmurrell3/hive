...
func TestUpdateUser(t *testing.T) {
	...
	if err := authorizer.UpdateUser("user-1", &newRole, newTeams, newAgents); err != nil {
		t.Fatalf("expected no error, got: %v", err)
	}
...
func TestUpdateUser_NotFound(t *testing.T) {
	...
	newRole := RoleAdmin
	if err := authorizer.UpdateUser("nonexistent", &newRole, nil, nil); err == nil {
...
func TestOperatorNoScopeHasFullAccess(t *testing.T) {
	t.Helper()

	// An operator with no teams or agents specified should have access to all resources.
	operator := &User{
	...
	if err := authorizer.Authorize(operator, "stop", "any-agent"); err != nil {
		t.Errorf("operator with no scope should have full access, got error: %v", err)
	}
}
