...
func TestGenerate_CreatesValidToken(t *testing.T) {
	...
	raw, err := Generate(store, 24*time.Hour)
	if err != nil {
...
func TestGenerate_MultipleTokens(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		_, err := Generate(store, time.Hour)
		if err != nil {
...
func TestValidate_FailsWithWrongToken(t *testing.T) {
	...
	_, err := Generate(store, 24*time.Hour)
	if err != nil {
		t.Fatalf("Generate: %v", err)
	}
...
func TestValidate_ExpiredTokenRejected(t *testing.T) {
	...
	raw, err := Generate(store, 1*time.Millisecond)
	if err != nil {
		t.Fatalf("Generate: %v", err)
	}
...
func TestGenerate_ProducesUniqueTokens(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		raw, err := Generate(store, time.Hour)
		if err != nil {
			t.Fatalf("Generate [%d]: %v", i, err)
		}
		if tokens[raw] {
