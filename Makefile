.PHONY: build build-linux-amd64 build-linux-arm64 build-darwin-amd64 build-all test test-unit test-integration test-vm test-e2e smoke soak test-all lint-full fuzz audit rootfs rootfs-openclaw download-kernel docker-test clean demo help
...
# Quick local smoke test: build, start cluster, run lifecycle, tear down (~30s)
smoke:
	@./scripts/smoke-test.sh

# Long-running stability test (default 5 min, override with SOAK_DURATION)
SOAK_DURATION ?= 300
soak:
	SOAK_DURATION=$(SOAK_DURATION) ./scripts/soak-test.sh

# Run everything: unit, integration, e2e, smoke
test-all: test test-e2e smoke

# Run tests inside a Docker container matching production (Ubuntu 22.04)
docker-test:
	docker build -f Dockerfile.test -t hive-test .
	docker run --rm hive-test
	docker run --rm hive-test make test-e2e
	docker run --rm hive-test ./scripts/smoke-test.sh
...
help: ## Show this help
	...
	@echo "  make smoke              Quick local smoke test (~30s)"
	@echo "  make soak               Stability soak test (SOAK_DURATION=300)"
	@echo "  make test-all           Run unit + integration + e2e + smoke"
	@echo "  make docker-test        Run all tests in Ubuntu 22.04 container"
