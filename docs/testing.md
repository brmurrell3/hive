...
# Monitor NATS subjects for sidecar heartbeats (use nats-cli or similar)
...
**Capability invoke times out**: Sidecar may not have finished connecting. Wait for a heartbeat to appear (use `nats sub "hive.health.>"`), then retry.
