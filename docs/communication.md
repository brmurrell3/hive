...
**Encoding:** gzip compression recommended.
...
hive.join.*                     [ephemeral]
hive.agent.*.inbox              [NOT persistent by default]
...
|------|-----------|---------|
| 1 VM | vsock | No TCP exposure, cluster-wide NATS auth token |
| 1 Native | IPC | Cluster-wide NATS auth token |
| 2 | TLS + Token | Control plane TLS, cluster-wide NATS auth token |

All agents share a single cluster-wide NATS auth token (auto-generated or configured via `nats.authToken` in cluster.yaml). Subject isolation is enforced by convention — agents publish/subscribe to their own `hive.agent.<id>.*` subjects.

### Authorization

- Application-layer RBAC (admin/operator/viewer) enforces access control on control plane operations
- Multi-user mode: per-user tokens scoped to specific teams/agents, enforced by the dashboard and control handler layer
- NATS transport layer uses a single shared auth token; per-agent NATS ACLs are not currently implemented
