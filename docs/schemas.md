...
spec:
  nats:
    ...
    clusterPort: int  # default 6222, NATS cluster peering (multi-node)
    jetstream:
...
spec:
  tier: enum(vm|native)  # OPTIONAL, auto-inferred if omitted
  ...
  resources:
    memory: string  # OPTIONAL. vm: Firecracker ceiling. native: scheduler hint.
    vcpus: int  # OPTIONAL, vm tier only
    disk: string  # OPTIONAL, default "5GB", vm tier only

  runtime:
    type: enum(openclaw|custom|process)  # REQUIRED
      # openclaw: OpenClaw agent runtime. Valid for vm, native.
      # custom: user-provided entrypoint.sh. Valid for vm, native.
      # process: process-based runtime. Valid for native.
    backend: enum(firecracker|process)  # OPTIONAL, default inferred from tier
      # firecracker: Firecracker VM backend. vm tier only.
      # process: process backend. native tier only.
  ...
  placement:
    ...
    arch: string  # OPTIONAL, enum: amd64, arm64, armv7, armv6
```
...
large_payload_strategy: for payloads exceeding max_inline_size, use reference mode:
  ...
  agent stores data locally (filesystem for vm/native)
  returns JSON: {"ref": "hive://AGENT_ID/blob/UUID", "size": int, "mime": string}
  consumer fetches via: sidecar GET /blobs/AGENT_ID/UUID
...
8. tier must be compatible with runtime type: vm -> openclaw, custom | native -> openclaw, custom, process
9. volumes only valid for vm tier
10. network egress only valid for vm tier
11. spec.models entries MUST NOT use reserved provider names: anthropic, openai, ollama, google, mistral, cohere
12. placement.nodeId if set must reference a registered node (warning, not error - node may join later)
13. user IDs in spec.users must be unique
14. user teams references must be valid team IDs or "all"
15. user agents references must be valid agent IDs
