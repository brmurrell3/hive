...
func TestServer_CORSHeaders(t *testing.T) {
	...
	// M-12: CORS headers are only set when an Origin header is present.
	req, err := http.NewRequest(http.MethodGet, ts.URL+"/api/cluster", nil)
	if err != nil {
	...
	// Verify CORS headers use the configured specific origin (not wildcard).
...
func TestServer_CORSHeaders_NoOrigin(t *testing.T) {
	ts, _ := setupTestServer(t)

	// M-12: Without Origin header, no CORS headers should be set.
	...
	if origin := resp.Header.Get("Access-Control-Allow-Origin"); origin != "" {
		t.Errorf("expected no Access-Control-Allow-Origin header without Origin request header, got %q", origin)
	}
}

func TestServer_CORSWildcardRejected(t *testing.T) {
	store := newMockStore()
	srv := NewServer(Config{
		Store:      store,
		Addr:       ":0",
	...
	// Wildcard CORS origin should be rejected; no Access-Control-Allow-Origin header.
	if origin := resp.Header.Get("Access-Control-Allow-Origin"); origin != "" {
		t.Errorf("expected no Access-Control-Allow-Origin header when wildcard is rejected, got %q", origin)
	}
}
...
func TestServer_CORSPreflight(t *testing.T) {
	...
	// M-12: CORS headers require an Origin header.
	req.Header.Set("Origin", "http://localhost:3000")
...
func TestWSHub_BroadcastNoClients(t *testing.T) {
	hub := newWSHub(slog.Default(), &mockStore{})
