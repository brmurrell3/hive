...
func TestRouter_InvocationSucceeds(t *testing.T) {
	...
	if err := routerB.RegisterHandler("echo", func(_ context.Context, inputs map[string]interface{}) (map[string]interface{}, error) {
...
func TestRouter_InvocationTimeout(t *testing.T) {
	...
	if err := routerB.RegisterHandler("slow", func(_ context.Context, inputs map[string]interface{}) (map[string]interface{}, error) {
		time.Sleep(10 * time.Second)
		return nil, nil
	}); err != nil {
		t.Fatalf("registering slow handler: %v", err)
	}
	...
	if resp.Error.Code != "HANDLER_TIMEOUT" {
		t.Errorf("expected error code HANDLER_TIMEOUT, got %q", resp.Error.Code)
	}
...
func TestRouter_EnvelopeFields(t *testing.T) {
	...
	_, err := ncB.Subscribe("hive.capabilities.agent-b.echo.request", func(msg *nats.Msg) {
		...
		respPayload, _ := marshalPayload(InvocationResponse{
			Capability: "echo",
			Status:     "success",
