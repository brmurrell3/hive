...
// then exercises hivectl commands, agent join, capability routing, health heartbeats,
// and the MQTT bridge — all without hardware or Docker.
func TestHiveE2E(t *testing.T) {
	...
	t.Run("mqtt_bridge", func(t *testing.T) {
		waitForPort(t, mqttPort, 5*time.Second)

		received := make(chan []byte, 1)
		sub, err := nc.Subscribe("hive.telemetry.sensor01", func(msg *nats.Msg) {
			received <- msg.Data
		})
		if err != nil {
		...
		conn, returnCode := mqttConnect(t, mqttPort, "sensor01", "sensor01", rawToken)
		if returnCode != 0 {
			t.Fatalf("MQTT CONNACK return code = %d, want 0", returnCode)
		}
		defer conn.Close()

		payload := []byte(`{"temp":22.5,"unit":"celsius"}`)
		pubPkt := buildMQTTPublish("hive/telemetry/sensor01", payload)
		if _, err := conn.Write(pubPkt); err != nil {
			t.Fatalf("sending MQTT PUBLISH: %v", err)
		}

		select {
		case data := <-received:
			if string(data) != string(payload) {
				t.Fatalf("received payload = %q, want %q", string(data), string(payload))
			}
			t.Logf("MQTT->NATS bridge working: %s", string(data))
		case <-time.After(5 * time.Second):
			t.Fatal("timed out waiting for MQTT->NATS bridged message")
		}
	})
}
...
func main() {
	...
	for _, ddl := range []string{
		...
		"CREATE TABLE IF NOT EXISTS agents (id TEXT PRIMARY KEY, team TEXT, status TEXT, node_id TEXT, memory_bytes INTEGER, vcpus INTEGER, manifest_hash TEXT, vm_pid INTEGER, vm_cid INTEGER, vm_socket_path TEXT, rootfs_copy_path TEXT, restart_count INTEGER DEFAULT 0, last_transition TEXT, started_at TEXT, error TEXT)",
